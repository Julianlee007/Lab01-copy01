...
```{r Ex_c}
visitors %>% arrange(desc(n_2022)) %>% head(n = 10)
```
...
```{r Ex_e}
visitors %>% filter(n_2022 > 1000000) %>% count()
```
...
```{r Ex_f}
visitors %>% 
  filter(n_2022 >= 50000,n_2022<= 100000) %>% count()
```
...
visitors %>% 
  summarise(
    mean_2021 = mean(na.rm=TRUE,n_2021),
    med_2021 = median(na.rm=TRUE,n_2021)
  )
```
...
**Not Applicable, because there are some empty values in n_2021**
...
**The command 'na.rm' ignores the NA(not applicable) values during calculations.**
...
**There are outliers that are really high in number of visitors**
...
visitors_with_nations %>% group_by(nation) %>% summarise(n_attractions = n())
```
